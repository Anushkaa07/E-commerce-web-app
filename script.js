...
// ---------- small preloader animations (assumes elements exist thanks to defer) ----------
...
// Close preloader after 2 seconds
...
const productData = {
    'C001': {
        ...
        category: 'keychains',
        // sale: true, // Add sale property
        // discountPercentage: 20, // Add discount percentage
        new: true // Add this property for New Arrivals
    },
    'C002': {
    ...
    'C037': {
        ...
        new: false // Add this property for New Arrivals
    },
    'C038': {
...
function openCartPage() {
    ...
    // Also hide category pages when opening cart
    document.getElementById('categoryPages').style.display = 'none';
...
function trapFocus(modal) {
    ...
    // store handler on modal so it can be removed later
    modal._trapFocusHandler = keyHandler;
}
function removeTrapFocus(modal) {
    if (modal && modal._trapFocusHandler) {
...
function productCardHtml(id, product) {
    ...
    // Generate sale badge if product is on sale
    const saleBadge = product.sale ? `<div class="sale-badge"><img src="images/1.jpg" alt="Sale" /></div>` : '';

    // Generate discount badge if there's a discount percentage
    const discountBadge = product.discountPercentage && product.discountPercentage > 0
        ? `<div class="discount-badge">${product.discountPercentage}% OFF</div>`
        : '';

    // Generate NEW badge if product is new (positioned at top center)
    const newBadge = product.new ? `<div class="new-badge"></div>` : '';
...
function populateProductModal() {
    ...
    // Add badges to main modal image
    const mainImageContainer = modalMainImage.parentElement;
    // Remove existing badges
    const existingSaleBadge = mainImageContainer.querySelector('.sale-badge');
    const existingDiscountBadge = mainImageContainer.querySelector('.discount-badge');
    ...
    if (currentProduct.sale) {
        ...
        saleBadge.innerHTML = '<img src="images/1.jpg" alt="Sale" />';
        mainImageContainer.appendChild(saleBadge);
    }

    // Add discount badge if there's a discount percentage
    if (currentProduct.discountPercentage && currentProduct.discountPercentage > 0) {
        const discountBadge = document.createElement('div');
        discountBadge.className = 'discount-badge';
        discountBadge.textContent = `${currentProduct.discountPercentage}% OFF`;
...
function updateCartPageDisplay() {
    ...
    let delivery = 0;
    const deliveryOptionTitle = document.getElementById('deliveryOptionTitle');
    const deliveryOptionPrice = document.getElementById('deliveryOptionPrice');
    if (deliveryOptionTitle && deliveryOptionPrice) {
        if (subtotal > 999) {
            deliveryOptionTitle.textContent = 'Standard Delivery (FREE)';
            deliveryOptionPrice.textContent = '₹0';
            delivery = 0;
        } else {
            deliveryOptionTitle.textContent = 'Standard (10-12 Days)';
            deliveryOptionPrice.textContent = '₹60';
            delivery = 60;
        }
    }
    ...
    // Overlay and button disable logic for minimum cart value
    const minCartValue = 249;
    const cartSummary = document.querySelector('.cart-summary');
    let overlay = document.getElementById('cartDisableOverlay');
    ...
    if (cartSummary) {
        ...
        if (subtotal < minCartValue) {
            ...
            overlay.className = 'cart-disable-overlay';
            overlay.id = 'cartDisableOverlay';
            overlay.innerHTML = `<div class="cart-disable-message">Minimum cart value is Rs 249</div>`;
            cartSummary.style.position = 'relative';
            cartSummary.appendChild(overlay);

            if (finalCheckoutBtn) {
                finalCheckoutBtn.disabled = true;
                finalCheckoutBtn.classList.add("disabled-proceed");
            }
        } else {
            if (finalCheckoutBtn) {
                finalCheckoutBtn.disabled = false;
                finalCheckoutBtn.classList.remove("disabled-proceed");
            }
        }
    }
    ...
    if (deliveryOptionsSection) {
        ...
        if (subtotal > 999) {
            if (!infoMsg) {
                ...
                infoMsg.textContent = '🎉 Standard delivery is FREE! ';
...
function handleFinalCheckout() {
    ...
    const messageText = message; // built string previously
...
function populatePriceOptions() {
    ...
    priceOptions.innerHTML = `
        <div class="price-card" data-price-range="under-150">
        <div class="price-icon"><i class="fas fa-coins"></i></div>
        <h3>Under ₹150</h3>
        <p>Budget-friendly range</p>
        <button class="price-btn" type="button">Shop Now</button>
        </div>
        <div class="price-card" data-price-range="150-400">
        <div class="price-icon"><i class="fas fa-money-bill-wave"></i></div>
        <h3>₹150 - ₹400</h3>
        <p>Perfect price range</p>
        <button class="price-btn" type="button">Shop Now</button>
        </div>
        <div class="price-card" data-price-range="above-400">
        <div class="price-icon"><i class="fas fa-crown"></i></div>
        <h3>Above ₹400</h3>
        <p>Premium collection</p>
        <button class="price-btn" type="button">Shop Now</button>
        </div>
        <div class="price-card" data-price-range="all">
        <div class="price-icon"><i class="fas fa-th-large"></i></div>
        <h3>Collection</h3>
        <p>Watch our entire collection</p>
        <button class="price-btn" type="button">Show all</button>
        </div>
    `;
}

// Delegate clicks once
document.querySelector('.price-options').addEventListener('click', (e) => {
    const card = e.target.closest('.price-card');
    if (!card) return;
    const range = card.dataset.priceRange;
    if (range === 'all') showAllCollection();
    else filterAllProductsByPrice(range);
});
...
// Add a new function to properly handle navigation states
...
function openCategoryPage(categoryKey) {
    ...
    // Save scroll position before hiding main content
    lastScrollY = window.scrollY;
    
    // Use the new showPage function
    showPage('categoryPages');
...
// --- Populate New Arrivals (Products marked as new) ---
function populateNewArrivals() {
    ...
    const newProducts = Object.entries(productData).filter(([id, product]) => product.new === true);

    // If no products are marked as new, show first 6 products as fallback
    const productsToShow = newProducts.length > 0 ? newProducts : Object.entries(productData).slice(0, 6);

    // Show all new products (or first 6 if no new products)
    productsToShow.forEach(([id, product]) => {
        grid.innerHTML += productCardHtml(id, product);
    });

    // Optional: Add a "View More" button if there are more than 6 new products
    if (newProducts.length > 6) {
        const viewMoreBtn = document.createElement('div');
        viewMoreBtn.className = 'view-more-wrapper';
        viewMoreBtn.innerHTML = `
            <button class="view-more-btn" onclick="showAllNewArrivals()">
                View All New Arrivals
            </button>
        `;
        grid.parentElement.appendChild(viewMoreBtn);
    }
}

// Function to show all new arrivals in a separate view
    ...
    <div class="category-page-bg">
      <div class="container">
        <div class="category-page-header">
            ...
            <span class="category-page-title">New Arrivals</span>
        </div>
        <div class="products-grid" id="categoryProductsGrid">
            ${newProducts.map(([id, p]) => productCardHtml(id, p)).join('')}
        </div>
      </div>
    </div>
    `;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Attach click delegation for price-options after DOM ready
...
window.showAllNewArrivals = showAllNewArrivals; // Add this line
